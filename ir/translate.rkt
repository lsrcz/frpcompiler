...
(define (translate inputs func-list start-input return-val should-emit-action inst)
  ...
  (define (translate-inst intro-mapping ref inst)
    ((cond [(return-val-found? return-val inst) translate-return-val-found]
  ...
  (define (translate-return-val-found intro-mapping ref inst)
    (let* ([shape (get-shape ref)]
           [missing (find-missing-var-deep (wrap-in-list shape) func-list return-val inst)])
      (match missing
        [(all-found)
         (list (ret-action-inst return-val inst ref))]
        [(not-found intros)
         (let* ([intros-shape (compute-intro-shape intros intro-mapping shape)]
                [new-intro-inst
                 (intro-inst
                  ...
                  intros-shape)]
                [new-ret-action-inst
                 (ret-action-inst return-val inst new-intro-inst)])
           (list new-intro-inst new-ret-action-inst))])))
  (define (translate-bind intro-mapping ref inst)
    (let* ([shape (get-shape ref)]
           [missing (find-missing-var (wrap-in-list shape) func-list return-val (bind-body inst))])
      (match missing
        [(all-found)
         (let ([new-inst
                (compute-inst
    ...
    (let* ([arg (return-arg inst)]
           ...
           [new-inst
            (if should-emit-action
                (ret-action-inst return-val (list 'return arg) next-ref)
...
(define (translate-spec spec-input)
  (define (gen-merge should-emit-retact inst-list)
    (let ([ret-list (filter (lambda (x) (if should-emit-retact (ret-action-inst? x) (ret-inst? x))) inst-list)])
      (if should-emit-retact
  ...
  (define (iter inputs output funclist should-emit-retact body)
    (define (translate-one inputs func-list output spec-one)
      (translate inputs func-list (car spec-one) output should-emit-retact (cadr spec-one)))
    (if (null? body)
        '()
        (append (translate-one inputs funclist output (car body)) (iter inputs output funclist should-emit-retact (cdr body)))))
  (match spec-input
    [(spec inputs output funclist body)
     (let ([should-emit-retact (find-retval-in-spec output (map cadr body))])
       (gen-merge should-emit-retact (iter inputs output funclist should-emit-retact body)))]))
...
(define (main)
               ...
               (bind _temp2
                     ...
                     (if-else _temp2
                              ...
                              (bind _temp1
                                    ...
                                    (return _temp1)))))))
  (define spec1 '(bind a (f (prev move)) (return a)))
  (define spec2 '(bind a (f move) (return a)))
  (define spec3 '(bind a (f move down) (return a)))
  ...
  (println "--2--")
  (print-inst-list (translate '(move) '(f) 'move 'drawing #f spec2))
  (println "--3--")
  (print-inst-list (translate '(move down) '(f) 'move 'drawing #f spec3))
  (println "--4--")
  (print-inst-list (translate '(move) '(f) 'move 'drawing #f spec4))
  (println "--5--")
