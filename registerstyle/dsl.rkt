...
(define (rxCombineLatest f inputStream1 inputStream2)
  (let ([stateStream1 (getStatefulStream (λ (evt accum)
    ...
    (for/list ([input-event1 inputStream1]
      ...
      (if (and (or (not-empty-event-reg? input-event1) (not-empty-event-reg? input-event2))
               (not-empty-event-reg? state-event1) (not-empty-event-reg? state-event2))
...
(define (rxFilter pred inputStream)
  (map (λ (evt) (if (or (empty-event-reg? evt) (not (pred evt)))
                    NOEVENT
                    evt)) inputStream))
...
(define (rxMerge stream1 stream2)
  (for/list ([elt1 stream1]
             [elt2 stream2])
    (if (not-empty-event-reg? elt1) elt1 elt2)))

;; http://reactivex.io/documentation/operators/buffer.html
(define (rxPairwise inputStream)
  (for/list ([i (range (length inputStream))])
    (if (or (empty-event-reg? (list-ref inputStream i))
            (andmap empty-event-reg? (take inputStream i)))
        NOEVENT
        (cons (foldl (λ (evt accum)
                       (if (and (empty-event-reg? accum) (not-empty-event-reg? evt))
                           evt
