...
(define (find-missing-var lst funclst return-val arg)
...
(define (find-missing-var-deep lst funclst return-val inst)
  (let* ([missing-in-arg
         ...
         [missing-in-next-inst-first
          (if (not (return? inst))
              (find-missing-var-deep new-lst funclst return-val
         ...
         [missing-in-next-inst-second
          (if (if-else? inst)
              (find-missing-var-deep new-lst funclst return-val (if-else-else-branch inst))
              (all-found))])
